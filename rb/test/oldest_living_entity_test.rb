...
class OldestLivingEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = oldest_living_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "oldest_living." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OLDEST_PEOPLE_RECORDS_TEST_OLDEST_LIVING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    oldest_living_ref01_data = nil
    if oldest_living_ref01_data_raw.length > 0
      oldest_living_ref01_data = Helpers.to_map(oldest_living_ref01_data_raw[0][1])
    end

    # UPDATE
    oldest_living_ref01_ent = client.OldestLiving(nil)
    ...
    oldest_living_ref01_resdata_up0 = Helpers.to_map(oldest_living_ref01_resdata_up0_result.respond_to?(:data_get) ? oldest_living_ref01_resdata_up0_result.data_get : oldest_living_ref01_resdata_up0_result)
    ...
    assert_equal oldest_living_ref01_resdata_up0[oldest_living_ref01_markdef_up0_name], oldest_living_ref01_markdef_up0_value
    ...
    oldest_living_ref01_data_dt0_load_result = Helpers.to_map(oldest_living_ref01_data_dt0_loaded.respond_to?(:data_get) ? oldest_living_ref01_data_dt0_loaded.data_get : oldest_living_ref01_data_dt0_loaded)
...
def oldest_living_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "oldest_living", "OldestLivingTestData.json")
  ...
  entid_env_raw = ENV["OLDEST_PEOPLE_RECORDS_TEST_OLDEST_LIVING_ENTID"]
