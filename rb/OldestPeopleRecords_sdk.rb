...
require_relative 'OldestPeopleRecords_types'
...
class OldestPeopleRecordsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OldestPeopleRecordsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OldestPeopleRecordsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OldestPeopleRecordsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OldestPeopleRecordsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OldestPeopleRecordsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OldestPeopleRecordsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OldestPeopleRecordsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => OldestPeopleRecordsError.new(
        "#{op}_allow",
        "OldestPeopleRecordsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue OldestPeopleRecordsError => err
    ...
    if fetched.is_a?(Hash)
      status = OldestPeopleRecordsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OldestPeopleRecordsError.new(
        "graphql_error", "OldestPeopleRecordsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OldestPeopleRecordsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
