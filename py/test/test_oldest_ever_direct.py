...
class TestOldestEverDirect:

    def test_should_direct_load_oldest_ever(self):
        setup = _oldest_ever_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-oldest_ever", "live" if setup["live"] else "unit")
        if _skip:
...
def _oldest_ever_direct_setup(mockres):
