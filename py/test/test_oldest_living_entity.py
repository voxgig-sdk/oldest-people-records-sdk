...
class TestOldestLivingEntity:

    def test_should_create_instance(self):
        testsdk = OldestPeopleRecordsSDK.test(None, None)
        ent = testsdk.OldestLiving(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _oldest_living_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "oldest_living." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OLDEST_PEOPLE_RECORDS_TEST_OLDEST_LIVING_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        oldest_living_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.oldest_living")))
        oldest_living_ref01_data = None
        if len(oldest_living_ref01_data_raw) > 0:
            oldest_living_ref01_data = helpers.to_map(oldest_living_ref01_data_raw[0][1])

        # UPDATE
        oldest_living_ref01_ent = client.OldestLiving(None)
        ...
        oldest_living_ref01_resdata_up0 = helpers.to_map(runner.entity_data(oldest_living_ref01_ent.update(oldest_living_ref01_data_up0_up, None)))
        ...
        oldest_living_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(oldest_living_ref01_data_dt0_loaded))
...
def _oldest_living_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/oldest_living/OldestLivingTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("OLDEST_PEOPLE_RECORDS_TEST_OLDEST_LIVING_ENTID"))
