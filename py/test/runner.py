# OldestPeopleRecords SDK test runner
...
class OldestPeopleRecordsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OldestPeopleRecordsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OldestPeopleRecordsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OldestPeopleRecordsTestRunner.getenv("OLDEST_PEOPLE_RECORDS_TEST_LIVE")
        override = OldestPeopleRecordsTestRunner.getenv("OLDEST_PEOPLE_RECORDS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OldestPeopleRecordsTestRunner.getenv(key)
        ...
        explain = OldestPeopleRecordsTestRunner.getenv("OLDEST_PEOPLE_RECORDS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if OldestPeopleRecordsTestRunner._test_control is not None:
            return OldestPeopleRecordsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OldestPeopleRecordsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OldestPeopleRecordsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OldestPeopleRecordsTestRunner.load_test_control()
...
def load_env_local():
    OldestPeopleRecordsTestRunner.load_env_local()


def env_override(m):
    return OldestPeopleRecordsTestRunner.env_override(m)


def entity_data(v):
    return OldestPeopleRecordsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OldestPeopleRecordsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OldestPeopleRecordsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OldestPeopleRecordsTestRunner.load_test_control()


def live_client_options():
    return OldestPeopleRecordsTestRunner.live_client_options()


def live_delay_ms():
    return OldestPeopleRecordsTestRunner.live_delay_ms()
