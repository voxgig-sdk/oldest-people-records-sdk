...
class TestOldestLivingDirect:

    def test_should_direct_load_oldest_living(self):
        setup = _oldest_living_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-oldest_living", "live" if setup["live"] else "unit")
        if _skip:
...
def _oldest_living_direct_setup(mockres):
