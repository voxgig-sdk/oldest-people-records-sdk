...
from oldestpeoplerecords_sdk.oldestpeoplerecords_types import (
    OldestLiving,
...
class OldestLivingEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OldestLivingEntity(self._client, opts)
    ...
    def data_get(self) -> OldestLiving:
    ...
    def match_get(self) -> OldestLiving:
    ...
    def load(self, reqmatch=None, ctrl=None) -> OldestLiving:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.OldestLiving().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: OldestLivingUpdateData, ctrl=None) -> OldestLiving:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
