// Command OldestPeopleRecords-cli is a CLI + REPL for the OldestPeopleRecords SDK,
...
const prompt = "oldest-people-records"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "oldest_ever oldest_living"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OLDEST_PEOPLE_RECORDS_APIKEY (api key), OLDEST_PEOPLE_RECORDS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OLDEST_PEOPLE_RECORDS_APIKEY carries the API key and
	// OLDEST_PEOPLE_RECORDS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OLDEST_PEOPLE_RECORDS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OLDEST_PEOPLE_RECORDS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOldestPeopleRecordsSDK(opts)
