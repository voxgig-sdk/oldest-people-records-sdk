...
boru-driven command-line client **and** interactive REPL for the OldestPeopleRecords
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/oldest-people-records-cli)
make build

# 2. See usage (words, entities, env vars)
./oldest-people-records-cli --help

# 3. Provide credentials once, via the environment
export OLDEST_PEOPLE_RECORDS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./oldest-people-records-cli load 1 oldest_ever            # {id:1} shorthand
./oldest-people-records-cli load '{id:1}' oldest_ever       # explicit match map
./oldest-people-records-cli update '{name:"x"}' oldest_ever

# 5. Override the API base URL for a single call
OLDEST_PEOPLE_RECORDS_BASE=https://api.example.com ./oldest-people-records-cli load 1 oldest_ever

# 6. No arguments -> interactive REPL
./oldest-people-records-cli
oldest-people-records> load 1 oldest_ever
oldest-people-records> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/oldest-people-records-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/oldest-people-records-cli load 1 oldest_ever
   ```
...
```sh
./oldest-people-records-cli load 1 oldest_ever          # scalar shorthand for {id:1}
./oldest-people-records-cli load '{id:1}' oldest_ever     # explicit match map
```
...
```sh
./oldest-people-records-cli update '{id:1,name:"new"}' oldest_ever
```
...
Run with no arguments to open a REPL (prompt `oldest-people-records>`). Each line is
evaluated as its own boru expression:

```text
$ ./oldest-people-records-cli
oldest-people-records> load 1 oldest_ever
oldest-people-records> /help
oldest-people-records> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `oldest_ever`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/oldest-people-records-cli`. |
...
(`./oldest-people-records-cli <expr>`) and interactively (the REPL), and expressions compose the
