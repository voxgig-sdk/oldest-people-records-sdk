# oldest-people-records-mcp

[MCP](https://modelcontextprotocol.io) server exposing the OldestPeopleRecords SDK as
two agent tools — `oldest-people-records_list` and `oldest-people-records_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/oldest-people-records-mcp)
make build

# 2. Provide credentials via the environment
export OLDEST_PEOPLE_RECORDS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user oldest-people-records \
  -- /absolute/path/to/oldest-people-records-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./oldest-people-records-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// oldest-people-records_list: first page of records
{ "entity": "oldest_ever" }
{ "entity": "oldest_ever", "query": { } }

// oldest-people-records_load: one record by id
{ "entity": "oldest_ever", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/oldest-people-records-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user oldest-people-records \
     -- "$PWD"/dist/*/oldest-people-records-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `oldest-people-records_list` and `oldest-people-records_load` tools now appear
   in new sessions. Ask the agent to *"list oldest_ever using oldest-people-records"*
   and it calls `oldest-people-records_list` with `{"entity":"oldest_ever"}`.
...
```sh
export OLDEST_PEOPLE_RECORDS_APIKEY=sk_live_xxx            # API key
export OLDEST_PEOPLE_RECORDS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `oldest-people-records_list` tool
...
### Call the `oldest-people-records_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `oldest-people-records_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `oldest-people-records_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OLDEST_PEOPLE_RECORDS_APIKEY` | API key sent with every request. |
| `OLDEST_PEOPLE_RECORDS_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

oldest_ever | oldest_living

### Smoke test via HTTP (raw JSON-RPC)

```sh
./oldest-people-records-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"oldest-people-records_load","arguments":{"entity":"oldest_ever","query":{"id":1}}}}'
```
