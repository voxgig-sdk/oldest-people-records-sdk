...
describe("OldestLivingEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set OLDEST_PEOPLE_RECORDS_TEST_OLDEST_LIVING_ENTID JSON to run live")
      return
    end
    ...
    local oldest_living_ref01_markdef_up0_name = "birthDate"
    ...
    assert.is_nil(err)
    local oldest_living_ref01_resdata_up0 = helpers.to_map(type(oldest_living_ref01_resdata_up0_result) == 'table' and oldest_living_ref01_resdata_up0_result.data_get and oldest_living_ref01_resdata_up0_result:data_get() or oldest_living_ref01_resdata_up0_result)
    ...
    assert.is_nil(err)
    local oldest_living_ref01_data_dt0_load_result = helpers.to_map(type(oldest_living_ref01_data_dt0_loaded) == 'table' and oldest_living_ref01_data_dt0_loaded.data_get and oldest_living_ref01_data_dt0_loaded:data_get() or oldest_living_ref01_data_dt0_loaded)
...
function oldest_living_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("OLDEST_PEOPLE_RECORDS_TEST_OLDEST_LIVING_ENTID")
  ...
  if env["OLDEST_PEOPLE_RECORDS_TEST_LIVE"] == "TRUE" then
