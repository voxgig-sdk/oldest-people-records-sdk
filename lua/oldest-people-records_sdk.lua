...
require("oldest-people-records_types")
...
local OldestPeopleRecordsSDK = {}
OldestPeopleRecordsSDK.__index = OldestPeopleRecordsSDK
...
OldestPeopleRecordsSDK._make_feature = _make_feature


function OldestPeopleRecordsSDK.new(options)
  local self = setmetatable({}, OldestPeopleRecordsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function OldestPeopleRecordsSDK:options_map()
...
function OldestPeopleRecordsSDK:get_utility()
  return Utility.copy(self._utility)
end


function OldestPeopleRecordsSDK:get_root_ctx()
  return self._rootctx
end


function OldestPeopleRecordsSDK:prepare(fetchargs)
...
function OldestPeopleRecordsSDK:direct(fetchargs)
...
function OldestPeopleRecordsSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function OldestPeopleRecordsSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "OldestPeopleRecordsSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function OldestPeopleRecordsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function OldestPeopleRecordsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "OldestPeopleRecordsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:OldestEver():list() / client:OldestEver():load({ id = ... })
...
-- Idiomatic facade: client:OldestLiving():list() / client:OldestLiving():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function OldestPeopleRecordsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = OldestPeopleRecordsSDK.new(sdkopts)
  sdk.mode = "test"
