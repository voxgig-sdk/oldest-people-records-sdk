...
describe('OldestLivingDirect', async () => {
  ...
  test('direct-load-oldest_living', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-oldest_living', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OLDEST_PEOPLE_RECORDS_TEST_OLDEST_LIVING_ENTID']
