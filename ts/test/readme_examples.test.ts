...
//     (\`new OldestPeopleRecordsSDK(...)\` / \`OldestPeopleRecordsSDK.test(...)\` become
//     \`OldestPeopleRecordsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OldestPeopleRecordsSDK'
...
const TEST_SEED = {"entity":{"oldest_ever":{"test01":{"id":"test01"}},"oldest_living":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OldestPeopleRecordsSDK(...)\` and any
// \`OldestPeopleRecordsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OldestPeopleRecordsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OldestPeopleRecordsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
