...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OLDEST_PEOPLE_RECORDS_TEST_LIVE") == "TRUE" ||
		os.Getenv("OLDEST_PEOPLE_RECORDS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OLDEST_PEOPLE_RECORDS_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.OldestPeopleRecordsSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.OldestPeopleRecordsError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.OldestPeopleRecordsSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.OldestPeopleRecordsError{Msg: msg, Code: code}
}
