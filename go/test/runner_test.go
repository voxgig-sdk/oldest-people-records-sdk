...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OLDESTPEOPLERECORDS_TEST_LIVE") == "TRUE" ||
		os.Getenv("OLDESTPEOPLERECORDS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OLDESTPEOPLERECORDS_TEST_EXPLAIN"); explain != "" {
