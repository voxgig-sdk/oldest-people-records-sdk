...
import (
	...
	omni "github.com/voxgig-sdk/oldest-people-records-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OldestPeopleRecordsError); is {
				out := map[string]any{
					"name":    "OldestPeopleRecordsError",
...
func omniCtx(arg any, client *sdk.OldestPeopleRecordsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OldestPeopleRecordsSDK); is {
			client = live
