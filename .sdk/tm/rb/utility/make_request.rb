...
module OldestPeopleRecordsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OldestPeopleRecordsResponse.new({})
    ...
    if fetch_err
      ...
      response = OldestPeopleRecordsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OldestPeopleRecordsResponse.new(fetched)
    else
