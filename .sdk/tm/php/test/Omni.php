...
//    only - a typed OldestPeopleRecordsContext there would make every ctx
...
final class OldestPeopleRecordsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new OldestPeopleRecordsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new OldestPeopleRecordsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new OldestPeopleRecordsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new OldestPeopleRecordsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new OldestPeopleRecordsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?OldestPeopleRecordsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OldestPeopleRecordsError(is_string($code) ? $code : '', $msg);
    }
