...
// OldestPeopleRecords SDK feature test
//
...
class FtCtrl extends OldestPeopleRecordsControl
{
...
class FtClient
{
    ...
    private OldestPeopleRecordsUtility $utility;

    public function __construct(OldestPeopleRecordsUtility $utility, array $options)
    {
    ...
    public function get_utility(): OldestPeopleRecordsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OldestPeopleRecordsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OldestPeopleRecordsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OldestPeopleRecordsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OldestPeopleRecordsConfig::make_feature($name);
    ...
    private static function buildUrl(OldestPeopleRecordsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OldestPeopleRecordsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OldestPeopleRecordsResponse($fetched);
        }
    ...
    private function fail(OldestPeopleRecordsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OldestPeopleRecordsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
