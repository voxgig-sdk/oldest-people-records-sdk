...
class OldestEverEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OLDEST_PEOPLE_RECORDS_TEST_OLDEST_EVER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $oldest_ever_ref01_resdata_up0_result = $oldest_ever_ref01_ent->update($oldest_ever_ref01_data_up0_up, null);
        $oldest_ever_ref01_resdata_up0 = Helpers::to_map(is_object($oldest_ever_ref01_resdata_up0_result) && method_exists($oldest_ever_ref01_resdata_up0_result, 'data_get') ? $oldest_ever_ref01_resdata_up0_result->data_get() : $oldest_ever_ref01_resdata_up0_result);
        ...
        $oldest_ever_ref01_data_dt0_loaded = $oldest_ever_ref01_ent->load($oldest_ever_ref01_match_dt0, null);
        $oldest_ever_ref01_data_dt0_load_result = Helpers::to_map(is_object($oldest_ever_ref01_data_dt0_loaded) && method_exists($oldest_ever_ref01_data_dt0_loaded, 'data_get') ? $oldest_ever_ref01_data_dt0_loaded->data_get() : $oldest_ever_ref01_data_dt0_loaded);
...
function oldest_ever_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("OLDEST_PEOPLE_RECORDS_TEST_OLDEST_EVER_ENTID");
