...
class OldestEverEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new OldestEverEntity($this->_client, $opts);
    }

    /**
     * @param OldestEver|array $args OldestEver data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OldestPeopleRecordsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return OldestEver|array The current OldestEver data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of OldestEver fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OldestPeopleRecordsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of OldestEver fields).
     */
     ...
     *
     * @param OldestEverLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed OldestEverLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return OldestEver|array The loaded OldestEver as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = OldestPeopleRecordsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param OldestEverUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed OldestEverUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return OldestEver|array The updated OldestEver as an assoc-array at the
     *   SDK boundary; throws OldestPeopleRecordsError on failure (item-5 convention).
     */
