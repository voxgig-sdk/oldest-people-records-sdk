...
// OldestPeopleRecords SDK retry feature
...
class OldestPeopleRecordsRetryFeature extends OldestPeopleRecordsBaseFeature
{
    ...
    public function init(OldestPeopleRecordsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OldestPeopleRecordsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OldestPeopleRecordsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OldestPeopleRecordsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
